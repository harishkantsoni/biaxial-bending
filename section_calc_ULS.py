...
def compute_capacities(xr, yr, Fr, Fc, sb_cog):
...
def compute_capacity_surface(x, y, xr, yr, fck, gamma_c, fyk, gamma_s, eps_cu,  rotation_step=5, vertical_step=10):
    ...
    for na_y in na_y_list:
        for alpha_deg in alpha_list:
            ...
            P, Mx, My = compute_capacities(xr, yr, Fr, Fc, sb_cog)
...
def plot_ULS_section(x, y, xr, yr, na_y, alpha_deg):
    ...
    rebars_inside = get_rebars_in_stress_block(xr, yr, x_sb, y_sb)
    P, Mx, My = compute_capacities(xr, yr, Fr, Fc, sb_cog)
    phi = compute_moment_vector_angle(Mx, My)
    C, T = compute_C_T_forces(Fc, Fr)         
    Mx_C, My_C, Mx_T, My_T = compute_C_T_moments(C, T, Mcx, Mcy, Mry, Mrx, Fr)
    ex_C, ey_C, ex_T, ey_T = compute_C_T_forces_eccentricity(C, T, My_C, Mx_C, Mx_T, My_T)
